...
export function managedPath(value) {
  artworkAssert(
    typeof value === "string" &&
      /^assets\/artworks\/(animation|chibi|stickers)-[0-9]+\/[a-f0-9]{64}\/(original\.(png|jpg|gif|mp4)|poster\.png|display\.(png|gif|mp4)|preview\.mp4)$/.test(
        value,
      ),
    "作品資源路徑不正確。",
  );
...
export async function readArtworkCatalog(raw, videos, directory = root) {
  ...
  const base = [
    ...raw.map((work) => ({
      ...work,
      previewSrc: videos.get(work.id).preview.src,
      playbackSrc: videos.get(work.id).display.src,
    })),
    stickerWork,
  ];
  ...
  for (const item of manifest.items) {
    ...
    if (item.media) {
      ...
      artworkAssert(
        ["image", "video"].includes(media.type) && media.width > 0 && media.height > 0,
        "作品媒體資訊不完整。",
      );
      artworkAssert(
        Array.isArray(media.assets) && media.assets.length >= 2,
        "作品缺少來源及衍生檔。",
      );
      for (const asset of media.assets) {
        const bytes = await readFile(
          resolveWithin(path.join(directory, "public"), managedPath(asset.path)),
        );
        artworkAssert(
      ...
      for (const key of [
        ...
        "poster",
        "playbackSrc",
        ...(media.type === "video" ? ["previewSrc"] : []),
      ]) {
        artworkAssert(
          media.assets.some((asset) => asset.path === media[key]),
          "作品媒體引用不完整。",
        );
      }
      artworkAssert(
        media.source?.path === media.src &&
          media.assets.some(
            (asset) =>
              asset.path === media.src &&
              asset.sha256 === media.source.sha256 &&
              asset.bytes === media.source.bytes,
          ),
        "作品原檔紀錄不完整。",
      );
    } else
      artworkAssert(
        base.some((work) => work.id === item.id),
        "新作品必須包含媒體。",
      );
  }
...
export function applyArtworkJob(manifest, base, job, media) {
  ...
  artworkAssert(
    (current?.revision || 0) === job.expectedRevision,
    "作品已被其他工作更新，請重新讀取後編輯。",
  );
  artworkAssert(!current?.deleted, "作品已下架。");
  artworkAssert(
    job.action === "delete" ? Boolean(current) : Boolean(current || media),
    "找不到作品媒體。",
  );

...
test("實際轉檔 PNG、JPG、GIF、MP4，原檔位元組不變、衍生檔可解碼且清單可重建", async () => {
  ...
  for (const [extension, type] of [
    ...
    await run(process.env.FFMPEG_PATH || "ffmpeg", [
      ...
      ...(extension === "mp4" || extension === "gif" ? ["-t", "0.4"] : ["-frames:v", "1"]),
      file,
    ]);
    const bytes = await readFile(file);
    const id = `chibi-${++index}`;
    const media = await processArtwork(
      bytes,
      { name: `fixture.${extension}`, type, size: bytes.length, sha256: sha256(bytes) },
      id,
      directory,
    );
    ...
    for (const asset of media.assets) {
      const metadata = JSON.parse(
        await run(process.env.FFPROBE_PATH || "ffprobe", [
          "-v",
          "error",
          "-show_streams",
          "-of",
          "json",
          path.join(directory, "public", asset.path),
        ]),
      );
  ...
  await writeFile(
    path.join(directory, "content/artworks.json"),

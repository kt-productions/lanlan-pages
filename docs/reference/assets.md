...
原站基底的以下五筆已有名稱，其他 89 筆使用類別與編號；基底名稱以 `content/works.json` 的 `title` 為準，實際顯示再套用 `content/artworks.json` 的覆寫。2026-09-24 依使用者提供的清單更新全部 60 件現行小動圖的名稱及順序，完整對照見[小動圖名稱與順序](../records/chibi-order-2026-09-24.md)，不把名稱退回初始盤點的編號。
...
舊盤點記錄原站播放器顯示預設的「Your Video Title」，因此初期採中性編號。現有 `title` 是本站顯示資料，不據此推定原站正式作品名、委託人或其他角色身分。首屏小動圖的替代說明與名稱沿用 `chibi-01` 的覆寫名稱，其餘首屏說明仍由模板與中央影片資料維護。

2026-09-24 使用者先確認作品編號越大越新，同日再提供小動圖的明確順序。作品牆優先按覆寫清單的 `sortOrder` 由大到小排列，未指定者沿用 ID 數字；同順位保留來源順序。各分類、載入更多、檢視器與管理清單共用此規則，不另推定完成日期，也不更動作品 ID。`content/site.json` 的 `featured` 保留原精選紀錄，但不再影響作品牆排序。首屏兩側固定為 `chibi-01`、`animation-01`；同日依使用者要求，中央改為新提供的「逼餔撩髮」GIF 衍生影片。

建置另加入 `stickers-01` 貼圖總覽，因此原站基底共 95 件，分類件數為角色動畫 37、小動圖 57、貼圖 1；實際發布件數須合併 `content/artworks.json` 的新增與下架；「貼圖 1」代表一張總覽，不代表只有一款可委託。依使用者同日後續要求，移除「全部作品」選項，預設顯示小動圖。
...
| `content/artworks.json`、`public/assets/artworks/` | 後台新增、文字／媒體覆寫及下架；原檔與衍生檔依作品 ID、來源雜湊保存，不改寫原站基底。 |
...
建置會複製 `public/`；原首頁來源 GIF 不打包，只有 `content/artworks.json` 明確引用的受管 GIF 會加入產物。`docs/` 不打包。2026-09-22 的歷史驗收數量反映當時內容，不能當成目前素材清單；現況以此表及 JSON 清單為準。

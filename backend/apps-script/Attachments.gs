...
function setupReferenceStorage() {
  ...
  Core_.requireValue(
    active && active === Session.getEffectiveUser().getEmail(),
    "請由專案編輯者執行附件儲存初始化。",
    "FORBIDDEN",
  );
  return lock_(function () {
    const parent = setting_("REFERENCE_FOLDER_ID", true)
      ? lanlanPagesDriveFolder_()
      : verifyLanlanPagesDriveFolder_();
...
function referenceFolder_() {
  ...
  Core_.requireValue(
    ...
    "圖片上傳尚未完成設定，請先提供參考素材連結。",
    "NOT_CONFIGURED",
  );
  const folder = JSON.parse(
    driveReferenceResponse_(
      id + "?fields=id,mimeType,trashed,parents,appProperties",
    ).getContentText(),
  );
  Core_.requireValue(
    !folder.trashed &&
      ...
      folder.appProperties?.lanlanReferenceStorage === "1",
    "附件資料夾設定不正確。",
    "CONFIG",
  );
  return id;
...
function parseReferenceUpload_(input, imagesOnly) {
  ...
  try {
    ...
    throw new Core_.OrderError("VALIDATION", "圖片編碼不正確。");
  }
  Core_.requireValue(
    bytes.length === input.size && Utilities.base64Encode(bytes) === input.base64,
    "圖片大小或編碼不正確。",
  );
  const unsigned = bytes.slice(0, 32).map(function (value) {
    return value & 255;
  });
  const text = unsigned
    .map(function (value) {
    ...
    .join("");
  const type =
    unsigned.slice(0, 8).join(",") === "137,80,78,71,13,10,26,10"
      ? "image/png"
      : unsigned[0] === 255 && unsigned[1] === 216 && unsigned[2] === 255
        ? "image/jpeg"
        : /^GIF8[79]a/.test(text)
          ? "image/gif"
          : text.slice(0, 4) === "RIFF" && text.slice(8, 12) === "WEBP"
            ...
            : text.slice(4, 8) === "ftyp" && /avif|avis/.test(text.slice(8))
              ...
              : "";
  Core_.requireValue(
    !Core_.ATTACHMENT_TYPES.includes(input.type) || type === input.type,
    "圖片內容與格式不符，請重新選擇有效圖片。",
  );
  ...
  return {
    ...
    size: bytes.length,
    bytes: bytes,
    storageType: type || "application/octet-stream",
    sha256: sha256,
...
function submissionInput_(payload) {
  ...
  const hashDetails =
    details.service === "animation" && details.estimatedPrice.currency === "TWD"
      ? Object.assign({}, details, {
          estimatedPrice: Object.assign({}, details.estimatedPrice, { currency: null }),
        })
      : details;
  return {
    details: details,
    ...
    hash: digest_(
      JSON.stringify(
        manifest.length ? { details: hashDetails, attachments: manifest } : hashDetails,
      ),
    ),
  };
}
...
function checkSubmissionCapacity_(orders, details, requestId) {
  ...
  const pending = Object.keys(props)
    .filter(function (key) {
      return key.startsWith("REFERENCE_UPLOAD_");
    })
    .map(function (key) {
      return Object.assign({ requestId: key.slice(17) }, JSON.parse(props[key]));
    })
    .filter(function (item) {
      return (
        ...
        !orders.some(function (order) {
          return order.requestId === item.requestId;
        })
      );
    });
  const limit = Number(setting_("DAILY_ORDER_LIMIT", true) || 50);
  Core_.requireValue(
    Number.isInteger(limit) && limit > 0 && recent.length + pending.length < limit,
    "今日收件已達上限，請稍後再試或聯絡繪師。",
    "RATE_LIMIT",
  );
  const same =
    recent.filter(function (order) {
      return (
...
function uploadOrderReference_(payload) {
  ...
  Core_.requireValue(
    Number.isInteger(payload.index) && payload.index >= 0 && payload.index < input.manifest.length,
    "附件位置不正確。",
  );
  const upload = parseReferenceUpload_(
    Object.assign({}, input.manifest[payload.index], { base64: payload.data }),
...
function storeReferenceUpload_(upload, payload, input) {
  ...
  if (reservation)
    Core_.requireValue(
      reservation.hash === input.hash,
      "同一送件識別碼的圖片或內容不同，請保留原內容重試。",
      "CONFLICT",
    );
  else {
    reservation = {
      hash: input.hash,
      contactHash: digest_(JSON.stringify(input.details.contact)),
      at: new Date().toISOString(),
      files: input.manifest.map(function () {
        return null;
      }),
    };
  }
  if (!reservation.files[payload.index]) {
    reservation.files[payload.index] = {
    ...
    : Drive.Files.create(
        {
          id: slot.id,
          name: upload.name,
          mimeType: upload.storageType,
          parents: [folder],
          appProperties: { lanlanRequest: payload.requestId, lanlanHash: input.hash },
        },
        Utilities.newBlob(upload.bytes, upload.storageType, upload.name),
        { fields: fields },
      );
  Core_.requireValue(
    !file.trashed &&
      file.parents?.includes(folder) &&
      file.appProperties?.lanlanHash === input.hash &&
      Number(file.size) === upload.size &&
      file.md5Checksum === upload.md5 &&
      file.mimeType === upload.storageType,
    "無法確認圖片保存結果，請保留原內容重試。",
    "ATTACHMENT",
  );
  reservation.files[payload.index] = {
    ...
    uploadedAt: reservation.at,
  };
  properties.setProperty(key, JSON.stringify(reservation));
}

function completedReferences_(requestId, input) {
  const saved = PropertiesService.getScriptProperties().getProperty(
    "REFERENCE_UPLOAD_" + requestId,
  );
  const reservation = saved ? JSON.parse(saved) : null;
  Core_.requireValue(
    reservation &&
      reservation.hash === input.hash &&
      reservation.files.length === input.manifest.length &&
      reservation.files.every(function (file, index) {
        return file && file.sha256 === input.manifest[index].sha256 && file.uploadedAt;
      }),
    "參考檔案尚未全部上傳完成，請保留此頁並重試。",
    "ATTACHMENT",
  );
...
function referenceBlob_(order, index) {
  ...
  Core_.requireValue(
    attachment && /^[A-Za-z0-9_-]+$/.test(attachment.id),
    "這筆委託沒有已上傳圖片。",
    "NOT_FOUND",
  );
  const folder = referenceFolder_();
  const file = JSON.parse(
    driveReferenceResponse_(
      attachment.id + "?fields=id,parents,size,mimeType,trashed",
    ).getContentText(),
  );
  Core_.requireValue(
    !file.trashed &&
      ...
      attachment.size <= Core_.ATTACHMENT_MAX_BYTES &&
      file.mimeType === attachment.type,
    "附件已變動或無法存取，請聯絡維護者。",
    "ATTACHMENT",
  );
  const bytes = driveReferenceResponse_(attachment.id + "?alt=media")
    .getBlob()
    .getBytes();
  Core_.requireValue(
    bytes.length === attachment.size && bytesDigest_(bytes) === attachment.sha256,
    "附件完整性檢查失敗，請聯絡維護者。",
    "ATTACHMENT",
  );
...
function adminReference_(payload) {
  Core_.requireValue(
    Number.isInteger(payload.index) &&
      payload.index >= 0 &&
      payload.index < Core_.ATTACHMENT_MAX_FILES,
    "附件位置不正確。",
  );
...
function referenceCaption_(order, names) {
  return (
    ...
    order.orderId +
    "\n類型：" +
    COMMISSION_CONFIG_.services[order.service].name +
    (names?.length ? "\n參考檔案：" + names.length + " 個" : "") +
...
function sendOrderNotification_(order, recipient, blob, index, reuse) {
  ...
  const heading =
    referenceCaption_(order, attachment ? [attachment.name] : []) +
    (attachment ? "\n附件：" + attachment.name : "");
  ...
  const method =
    ...
    (["image/jpeg", "image/png", "image/webp"].includes(attachment.type)
      ...
      : attachment.type === "image/gif"
        ? "sendAnimation"
        : "sendDocument");
  const field = { sendPhoto: "photo", sendAnimation: "animation", sendDocument: "document" }[
    method
  ];
...
function sendReferenceBatch_(order, recipient, indices, reusable) {
  if (indices.length === 1) {
    ...
    return sendOrderNotification_(
      order,
      recipient,
      reusable[index] ? null : referenceBlob_(order, index),
      index,
      reusable[index],
    );
  }
  ...
  if (
    indices.every(function (index) {
      return reusable[index]?.method === "sendDocument";
    })
  )
    type = "document";
  const blobs = {};
  function send(kind) {
    ...
    const media = indices.map(function (index, position) {
      ...
      if (position === 0)
        item.caption = referenceCaption_(
          order,
    ...
    return UrlFetchApp.fetch(
      "https://api.telegram.org/bot" + setting_("TELEGRAM_BOT_TOKEN") + "/sendMediaGroup",
      {
        method: "post",
        muteHttpExceptions: true,
        followRedirects: false,

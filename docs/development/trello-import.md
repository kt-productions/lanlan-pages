...
2026-09-23 使用者指定三個公開看板，並確認匯入全部卡片（包含封存）及沿用原公開名稱。新表單仍保持匿名。這是一次性移轉，不會持續與 Trello 同步。
...
2. 執行 `npm test`、`npm run build:backend`。上傳最新來源並由專案編輯者執行 `setupOrders()`；現有 27／29／30 欄僅追加至 31 欄，資料或公式占用新增欄時停止。

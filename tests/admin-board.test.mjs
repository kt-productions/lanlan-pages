...
test("管理看板讀完所有分頁才提供完整快照，後續頁可搜尋且件數符合旗標", async () => {
  const source = Array.from({ length: 65 }, (_, index) => ({
    orderId: `LL-${index}`,
    details: { nickname: `測試 ${index}` },
    service: "chibi",
    status: index < 60 ? "delivered" : "queued",
    isRush: index === 64,
    isOnHold: index === 64,
  }));
  const offsets = [];
  ...
  const snapshot = await loadBoardOrders(
    async (action, { offset, delivery }, token) => {
      assert.equal(action, "admin.list");
      assert.equal(delivery, "all");
      assert.equal(token, "fixture-session");
      offsets.push(offset);
      return {
        orders: source.slice(offset, offset + 30),
        total: 65,
        nextOffset: offset + 30 < 65 ? offset + 30 : null,
      };
    },
    {
      admin: true,
...
test("管理看板拒絕循環游標、重複而缺漏的訂單，空清單可正常完成", async () => {
  ...
  await assert.rejects(
    loadBoardOrders(async () => ({
      orders: [{ orderId: "LL-1" }, { orderId: "LL-1" }],
      total: 2,
      nextOffset: null,
    })),
  );
  assert.deepEqual(
    await loadBoardOrders(async () => ({ orders: [], total: 0, nextOffset: null })),
    [],
  );
});

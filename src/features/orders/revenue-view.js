...
export function setupRevenueReport({ api, getToken, report, beforeSwitch, openOrder }) {
  ...
  // 收益屬於委託管理，從工作區開啟；作品管理維持獨立頁面。
  const button = $("admin-revenue");
  const back = $("revenue-back");
  let snapshot = null;
  let generation = 0;
  let busy = false;
  let selectedYear = Number(taipeiDate(new Date().toISOString()).slice(0, 4));
  let lastTrigger = back;
  ...
  back.addEventListener("click", () => beforeSwitch(() => {
    panel.hidden = true;
  ...
  button.addEventListener("click", () => beforeSwitch(() => {
    ...
    panel.hidden = false;
    $("revenue-heading").focus({ preventScroll: true });
    load();
  }));
  ...
  return {
    ...
    focus() { (lastTrigger.isConnected ? lastTrigger : back).focus({ preventScroll: true }); },
    invalidate() {
    ...
    clear() {
      generation++; snapshot = null; busy = false; panel.hidden = true;
      panel.setAttribute("aria-busy", "false");
      ...
      $("revenue-content").hidden = true; lastTrigger = back;
    },
  };
}

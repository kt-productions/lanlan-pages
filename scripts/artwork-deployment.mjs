...
artworkAssert(
  ...
  "只允許 production 的指定版本部署。",
);
async function current() {
  const response = await fetch(
    `https://api.github.com/repos/${process.env.GITHUB_REPOSITORY}/git/ref/heads/production`,
    {
      headers: {
...
if (process.argv[2] === "check") {
  ...
  if (await current()) {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      if (data?.commitSha === process.env.GITHUB_SHA) {
        verified = true;
        break;
      }
    ...
    if (process.env.ARTWORKS_ENABLED === "true" && (await current())) {
      try {
        await workerClient()("deployed", { commitSha: process.env.GITHUB_SHA });
      } catch (error) {

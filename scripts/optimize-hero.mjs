...
const probe = (file) =>
  JSON.parse(run(ffprobe, ["-v", "error", "-show_streams", "-show_frames", "-of", "json", file]));
...
assert.deepEqual(
  result.frames.map((frame) => Number(frame.pts_time)),
  input.frames.map((frame) => Number(frame.pts_time)),
  "影格時間戳改變",
);
...
console.log(
  `首頁中央影片：${original.length} → ${bytes.length} bytes；縮圖 ${posterBytes.length} bytes，原始 GIF 保留。`,
);

...
export function renderReview(config, snapshot) {
  ...
  const rows = [
    ...
    ["聯絡方式", contactNames[snapshot.contact.channel] + " · " + snapshot.contact.value],
  ...
  if (service === "stickers")
    rows.push(["貼圖款式", snapshot.stickerIds.map((id) => "No." + id).join("、")]);
  if (service === "chibi")
    rows.push(["委託方案", snapshot.chibiPlan === "animated" ? "插圖＋動畫" : "插圖"]);
  ...
  if (snapshot.commercial !== null) rows.push(["商業用途", snapshot.commercial ? "是" : "否"]);
  if (snapshot.background !== null)
    rows.push(["背景與特效", snapshot.background ? "需要" : "單色／無背景"]);
  if (snapshot.rush !== null) rows.push(["急件", snapshot.rush ? "是，需討論" : "否"]);
    ...
    snapshot.estimatedPrice.items
      .map(
        (item) =>
          item.label +
          "：" +
          formatPriceRange(item.min, item.max, snapshot.estimatedPrice.currency),
      )
      .join("\n"),
  ]);

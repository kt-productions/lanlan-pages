...
function verifyLanlanPagesDriveFolder_() {
  ...
  const folder = JSON.parse(
    driveReferenceResponse_(id + "?fields=id,mimeType,trashed").getContentText(),
  );
  Core_.requireValue(
    !folder.trashed && folder.mimeType === LANLAN_PAGES_FOLDER_MIME_,
    "LanLan Pages Drive 上層資料夾設定不正確。",
    "CONFIG",
  );
  return id;

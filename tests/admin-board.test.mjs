...
import { loadAdminOrders } from "../src/features/orders/admin-data.js";
...
test("管理看板讀完所有分頁才提供完整快照，後續頁可搜尋且件數符合旗標", async () => {
  ...
  const offsets = [];
  const progress = [];
  const snapshot = await loadAdminOrders(async (action, { offset }, token) => {
  ...
  }, "fixture-session", (loaded, total) => progress.push([loaded, total]));
...
test("分頁失敗或總數變動不回傳部分看板，也不在讀取中寫入訂單", async () => {
  ...
  for (const second of [new Error("讀取失敗"), { orders: [], total: 3, nextOffset: null }]) {
    await assert.rejects(loadAdminOrders(async (action, { offset }) => {
      assert.equal(action, "admin.list");
...
test("管理看板拒絕循環游標、重複而缺漏的訂單，空清單可正常完成", async () => {
  await assert.rejects(loadAdminOrders(async () => ({ orders: [], total: 1, nextOffset: 0 }), "fixture"));
  await assert.rejects(loadAdminOrders(async () => ({ orders: [{ orderId: "LL-1" }, { orderId: "LL-1" }], total: 2, nextOffset: null }), "fixture"));
  assert.deepEqual(await loadAdminOrders(async () => ({ orders: [], total: 0, nextOffset: null }), "fixture"), []);
});

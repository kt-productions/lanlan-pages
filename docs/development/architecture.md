...
`scripts/build.mjs` 讀取內容 JSON，排序作品、加入貼圖總覽，排除尚待確認的貼圖選項並解析款式價格，再組合 HTML 模板與共用導覽。主要導覽為「首頁、委託表單、委託進度」，已驗證且未到期的 Telegram 登入另顯示「委託管理、作品管理」。兩個管理入口分別連到 `admin/` 與 `artworks/`，由同一個權限狀態控制。

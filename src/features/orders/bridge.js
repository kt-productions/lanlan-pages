    ...
    return Boolean(
      event.source &&
        (event.source === frameWindow ||
          event.source.parent === frameWindow ||
...
export function createBridge(apiUrl, host = window, doc = document) {
    ...
    connection = new Promise((resolve, reject) => {
      ...
      const receive = (event) => {
        ...
        if (message.type === "lanlan:ready" && !peer) {
          ...
          try {
            task.resolve(JSON.parse(message.result));
          } catch {
            task.reject(new Error("服務回應格式不正確"));
          }
        }
      };
      dispose = () => {
  ...
  return async (request) => {
    ...
    if (
      text.length >
      (["orders.upload", "admin.artworks.upload"].includes(request.action)
        ? API_MAX_REQUEST_CHARS
        : 40000)
    )
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = host.setTimeout(
        ...
        request.action.startsWith("admin.artworks.") ||
          [
            "orders.upload",
            "orders.submit",
            "admin.attachment",

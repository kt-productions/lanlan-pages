...
const isRevenueView = new URLSearchParams(location.search).get("view") === "revenue";
if (isRevenueView) {
  document.title = "收益報表｜爛爛 LANLAN";
  document.querySelector("h1").textContent = "收益報表";
  document.querySelector(".order-heading p").textContent = "每份委託，從預計收益到完成交稿。";
  document.querySelector("#admin-login-panel h2 + p").textContent =
    "僅限指定的管理員使用。完成 Telegram 身分驗證後，可以查看收益報表及編輯訂單金額與收款日期。";
}
...
const attachments = setupAttachments(
  document.querySelector("#edit-attachments"),
  api,
  () => token,
  report,
);
...
const revenue = setupRevenueReport({
  ...
  openOrder(orderId) {
    afterDiscard(async () => {
      let order;
      await work(async () => {
        order = await api("admin.get", { orderId }, token);
      });
...
dialog.addEventListener("close", () => {
  ...
  const trigger = [...list.querySelectorAll("[data-order-id]")].find(
    (card) => card.dataset.orderId === returnOrderId,
  );
...
function renderList() {
  ...
  const scrollPositions = new Map(
    [...list.querySelectorAll(".board-column")].map((column) => [
      column.className,
      column.querySelector(".column-cards").scrollTop,
    ]),
  );
  list.replaceChildren(
    ...boardColumns({
  ...
  const scopeTotal = filterBoard(orders, {
    flag: flag.value === "archived" ? "archived" : "",
  }).total;
  const scopeLabel = `${flag.value === "archived" ? "封存" : "未封存"}${deliveredLoaded ? "" : "、未交稿"}委託`;
  summary.textContent = !hasSnapshot
    ? ""
    : result.total
...
async function load(includeDelivered = false) {
  ...
  renderList();
  message(
    `已載入 ${snapshot.length} 件${label}委託。${includeDelivered ? "" : "已交稿可按需載入。"}`,
  );
}
login.addEventListener("click", () =>
  work(async () => {
    ...
    const browserKey = [...bytes].map((value) => value.toString(16).padStart(2, "0")).join("");
...
function loadDelivered() {
  afterDiscard(async () => {
    ...
    if (token) {
      const target =
        list.querySelector(".column-delivered .column-cards > button") ||
...
for (const filter of [flag, service])
...
form.addEventListener("submit", (event) => {
  ...
  if (invalid) {
    message(
      `請檢查「${invalid.closest("label")?.firstChild.textContent.trim() || "必填欄位"}」的格式與填寫內容。`,
    );
    invalid.focus();
    return;
  }
  ...
  work(async () => {
    ...
    revenue.invalidate();
    // 共用編輯器可開啟未載入看板的訂單；更新目前快照時仍須遵守交稿範圍。
    orders = orders.filter((item) => item.orderId !== order.orderId);
...
retry.addEventListener("click", () => {
  ...
  work(async () => {
    ...
    const result = await api("admin.retryNotification", { orderId: selected.orderId }, token);
...
async function exchangeLogin() {
  ...
  if (!persisted && token)
    message(`${status.textContent} 瀏覽器無法保存登入，關閉或重新整理後需再登入。`);
}
function activateSession(session) {
  ...
  workspace.hidden = isRevenueView;
  logout.hidden = false;
}

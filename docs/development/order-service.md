...
- 管理 API 每頁 30 筆，管理頁依序讀完全部分頁後呈現六欄看板；編號／暱稱搜尋及類型／旗標篩選涵蓋全部已取得資料，預設小動圖，沒有上方階段篩選。桌機拖曳卡片可更新階段，手機與鍵盤可用卡片「編輯」視窗。公開看板每次最多 200 筆，回傳各階段總數，類型／階段／旗標篩選涵蓋所有工作，包含已交稿。直接掃描 Orders 適合小型工作室，不適合大量訂單。

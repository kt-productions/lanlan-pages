...
const env = { ARTWORK_API_URL: "https://script.google.com/macros/s/test/exec", ARTWORK_SITE_ID: "test", ARTWORK_WORKER_SECRET: "a".repeat(48) };
...
test("GAS 結果轉址只送 GET，HTML 或暫時錯誤只重讀結果、不重送寫入", async () => {
  const calls = [];
  const responses = [new Response(null, { status: 302, headers: { location: resultUrl } }),
    new Response("<html>temporary</html>"), new Response("unavailable", { status: 503 }), Response.json({ ok: true, data: { job: null } })];
  const client = workerClient(env, async (url, options) => { calls.push({ url, options }); return responses.shift(); }, async () => {});
...
test("拒絕非 Google 結果網址，服務錯誤不洩漏 HTML 或結果票證", async () => {
  for (const location of ["https://example.com/result", "http://script.googleusercontent.com/result", "https://user:password@script.googleusercontent.com/result"]) {
    let calls = 0;

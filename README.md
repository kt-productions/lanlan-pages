...
四頁的主要導覽會在本站 Telegram 登入通過伺服器管理權限確認後顯示「委託管理」。登出、到期或權限失效會隱藏入口；驗證暫時失敗時也先隱藏，保留登入供恢復連線後重試。沿用原三天期限，不因瀏覽頁面續期；驗證見[管理導覽紀錄](docs/records/admin-navigation-2026-09-24.md)。
...
已在本機來源新增管理員「收益報表」：每月及全年區分真實收益、暫收訂金與未完成餘額，依交稿／收款／預計交稿日期歸屬；交稿後不重複計入訂金。缺少金額或日期另列待補，可直接開啟訂單修正。這是目前訂單狀態的快照，規格及發布順序見[收益報表](docs/reference/revenue-report.md)；本次功能尚未部署至正式環境。

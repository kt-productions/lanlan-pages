...
export const artworkPng = Buffer.from(
...
export function artworkPayload(overrides = {}) {
  return {
    ...
    file: {
      name: "虛構作品.png",
      type: "image/png",
      size: artworkPng.length,
      sha256: sha256(artworkPng),
    },
    ...overrides,
  };
}
export function artworkBackend() {
  ...
  context.SpreadsheetApp.openById = (...args) => ({
    getSheetByName: (name) =>
      name === "ArtworkJobs" ? sheet : openBook(...args).getSheetByName(name),
    insertSheet: (name) => (name === "ArtworkJobs" ? sheet : openBook(...args).insertSheet(name)),
  });
  context.Utilities.computeRsaSha256Signature = (value, key) => [
    ...sign("RSA-SHA256", Buffer.from(value), key),
  ];
  for (const [key, value] of Object.entries({
    ARTWORKS_ENABLED: "true",
    ARTWORK_SITE_ID: "fixture",
    ARTWORK_GITHUB_REPO: "fixture/site",
    ARTWORK_GITHUB_APP_ID: "123",
    ARTWORK_GITHUB_INSTALLATION_ID: "456",
    ARTWORK_GITHUB_PRIVATE_KEY: signingKeys.privateKey.export({ type: "pkcs1", format: "pem" }),
    ARTWORK_WORKER_SECRET: "fixture-worker-secret-not-for-production",
    LANLAN_PAGES_DRIVE_FOLDER_ID: "fixture-lanlan-pages-folder",
    ARTWORK_FOLDER_ID: "fixture-artwork-folder",
  }))
    properties.set(key, value);
  files.set("fixture-artwork-folder", {
    id: "fixture-artwork-folder",
  ...
  const remote = {
    ...
    commits: new Map([["f".repeat(40), { manifest: { version: 1, items: [] } }]]),
    main: "f".repeat(40),
    production: "0".repeat(40),
    failDispatch: false,
    wrongBytes: false,
  };
  ...
  context.UrlFetchApp.fetch = (url, options = {}) => {
    if (!url.startsWith("https://api.github.com/")) {
      if (options.headers?.Range && url.includes("alt=media")) {
        ...
        return {
          ...response(206, {}),
          getBlob: () => context.Utilities.newBlob(file.bytes.slice(start, end + 1)),
        };
      }
    ...
    if (url.endsWith("/dispatches"))
      return response(remote.failDispatch ? 500 : 200, { workflow_run_id: 123 });
    ...
    if (url.endsWith("/git/ref/heads/production"))
      return response(200, { object: { sha: remote.production } });
    if (url.endsWith("/git/ref/heads/main")) return response(200, { object: { sha: remote.main } });
    if (url.includes("/git/commits/"))
      return response(200, { tree: { sha: url.split("/").at(-1) } });
    if (url.includes("/git/trees/")) {
      const commit = remote.commits.get(new URL(url).pathname.split("/").at(-1));
      return response(200, {
        tree: commit.manifest.items.flatMap((item) =>
          item.media
            ? [
                {
                  path: "public/" + item.media.source.path,
                  sha: item.media.source.sha256,
                  size: item.media.source.bytes,
                  type: "blob",
                },
              ]
            : [],
    ...
    if (url.includes("/git/blobs/"))
      return response(200, {
        encoding: "base64",
        content: (remote.wrongBytes ? Buffer.alloc(artworkPng.length) : artworkPng).toString(
          "base64",
        ),
      });
    return response(404, {});
  ...
  const admin = (action, payload = {}) =>
    service.invoke("admin.artworks." + action, payload, service.session());
  const machine = (action, payload = {}, envelopeChange = {}) => {
    ...
    const signature = createHmac("sha256", properties.get("ARTWORK_WORKER_SECRET"))
      .update(`${timestamp}.${nonce}.${body}`)
      .digest("hex");
    return service.invoke("artworks.worker", {
      body,
      timestamp,
      nonce,
      signature,
  ...
  function commit(job) {
    ...
    const original = {
      path: `assets/artworks/${job.work.id}/${job.file?.sha256}/original.png`,
      bytes: job.file?.size,
    ...
    const item = {
      ...job.work,
      revision: job.expectedRevision + 1,
      ...
      ...(job.file
        ? {
            media: {
              ...
              height: 1,
              src: original.path,
              poster: original.path,
              playbackSrc: original.path,
              source: original,
              assets: [original],
            },
          }
        : previous?.media && job.action !== "delete"
          ? { media: previous.media }
          : {}),
    };
    remote.manifest = {
      version: 1,
      items: [...remote.manifest.items.filter((entry) => entry.id !== item.id), item],
    };

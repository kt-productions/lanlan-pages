...
export function workerClient(env = process.env, fetcher = fetch, wait = setTimeout) {
  ...
  artworkAssert(
    env.ARTWORK_WORKER_SECRET?.length >= 32 && /^[a-z0-9-]{1,60}$/.test(env.ARTWORK_SITE_ID || ""),
    "作品工作憑證尚未設定。",
  );
  return async function call(action, payload = {}) {
    ...
    const signature = createHmac("sha256", env.ARTWORK_WORKER_SECRET)
      .update(`${timestamp}.${nonce}.${body}`)
      .digest("hex");
    const signal = AbortSignal.timeout(240000);
    const envelope = JSON.stringify({
      action: "artworks.worker",
      payload: { body, timestamp, nonce, signature },
    });
    let result;
    for (let delivery = 0; delivery < 3 && !signal.aborted; delivery++) {
      ...
      let response;
      try {
        response = await fetcher(env.ARTWORK_API_URL, {
          method: "POST",
          redirect: "manual",
      ...
      if ([302, 303].includes(response.status)) {
        try {
          resultUrl = new URL(response.headers.get("location"));
        } catch {
          /* 不輸出包含結果票證的網址。 */
        }
        artworkAssert(
          resultUrl?.protocol === "https:" &&
            resultUrl.hostname === "script.googleusercontent.com" &&
            !resultUrl.username &&
            !resultUrl.password &&
            !resultUrl.port,
          "作品服務回應的轉址不正確。",
      ...
      for (let attempt = 0; attempt < (resultUrl ? 3 : 1) && !signal.aborted; attempt++) {
        ...
        if (resultUrl) {
          try {
            response = await fetcher(resultUrl.href, { method: "GET", redirect: "error", signal });
          } catch {
            continue;
          }
        }
...
export async function installationToken(env = process.env, fetcher = fetch) {
  artworkAssert(
    /^\d+$/.test(env.ARTWORK_APP_ID || "") &&
      /^\d+$/.test(env.ARTWORK_INSTALLATION_ID || "") &&
  ...
  const body = [
    ...
    Buffer.from(
      JSON.stringify({ iat: now - 60, exp: now + 540, iss: env.ARTWORK_APP_ID }),
    ).toString("base64url"),
  ].join(".");
  const jwt = `${body}.${sign("RSA-SHA256", Buffer.from(body), env.ARTWORK_APP_PRIVATE_KEY.replace(/\\n/g, "\n")).toString("base64url")}`;
  const response = await fetcher(
    `https://api.github.com/app/installations/${env.ARTWORK_INSTALLATION_ID}/access_tokens`,
    {
      method: "POST",
      headers: {

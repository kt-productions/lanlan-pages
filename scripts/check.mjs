...
const works = JSON.parse(await readFile(path.join(root, "content/works.json"), "utf8"));
const site = JSON.parse(await readFile(path.join(root, "content/site.json"), "utf8"));
const videoAssets = await readVideoAssets(works);
const { works: catalogWorks, manifest: artworkManifest } = await readArtworkCatalog(
  works,
  videoAssets,
);
...
assert.equal(new Set(works.map((w) => w.id)).size, works.length, "作品 ID 不可重複");
...
for (const work of works) {
  ...
  for (const field of ["src", "poster"]) {
    ...
    assert.ok(resolved.startsWith(path.join(root, "public") + path.sep), "資源不可跳出 public");
    const info = await stat(resolved);
    assert.ok(info.size > 0 && info.size < 100 * 1024 * 1024, `${work.id} ${field} 檔案大小不合適`);
  }
  assert.ok(work.width > 0 && work.height > 0 && work.duration > 0, `${work.id} 媒體資訊有誤`);
}
for (const entry of [...site.services.map((s) => s.form), ...site.socials.map((s) => s.url)])
...
const commissionHtml = await readFile(path.join(root, "dist/commission/index.html"), "utf8");
...
for (const [file, page] of Object.entries(pages)) {
  ...
  for (const [, resource, fragment] of page.matchAll(
    ...
    const target = resolveWithin(output, path.join(path.dirname(file), resource));
    const info = await stat(target);
    const targetFile = info.isDirectory() ? path.join(target, "index.html") : target;
    assert.ok((await stat(targetFile)).isFile(), `${file} 資源不是檔案：${resource}`);
  ...
  assert.ok(new URL(canonical).pathname.endsWith("/"), `${file} canonical 必須是目錄網址`);
  if (file !== "index.html") {
    const legacy = await readFile(path.join(output, path.dirname(file) + ".html"), "utf8");
  ...
  assert.ok(
    !/<a\b[^>]*href="https:\/\/(?:forms\.gle|docs\.google\.com\/forms)/.test(page),
    "新版不可再連往舊 Google 表單",
  );
}
assert.ok(
...
for (const [video] of html.matchAll(/<video\b[^>]*>/g)) {
  for (const attribute of ["data-autoplay", "muted", "loop", "playsinline"])
    assert.match(video, new RegExp(`\\b${attribute}(?:\\s|>)`), `影片缺少 ${attribute}`);
  assert.ok(!/\ssrc=|\sautoplay(?:\s|>)/.test(video), "背景影片必須由可見性控制載入與播放");
  assert.ok(video.includes('preload="none"'), "背景影片不可預載");
  assert.ok(
    video.includes("assets/videos/optimized/") || video.includes("/preview.mp4"),
    "背景影片必須使用壓縮版",
  );
  if (video.includes("data-poster=")) assert.ok(!/\sposter=/.test(video), "作品縮圖必須延遲載入");
}
...
for (const work of portfolio.works.filter((item) => item.type === "video"))
  assert.equal(
    work.playbackSrc,
    catalogWorks.find((item) => item.id === work.id).playbackSrc,
    "檢視器必須使用壓縮版",
  );
const previewSources = new Set([...videoAssets.values()].map((item) => `./${item.preview.src}`));
for (const work of catalogWorks.filter((item) => item.type === "video"))
  previewSources.add(`./${work.previewSrc}`);
...
const formAssets = JSON.parse(await readFile(path.join(root, "content/form-assets.json"), "utf8"));
...
async function checkModules(directory) {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    assert.ok([".js", ".css"].includes(path.extname(file)), `不應輸出來源模板：${file}`);
    ...
    if (file.endsWith(".js")) {
      ...
      for (const [, imported] of source.matchAll(
        ...
        assert.ok(imported.startsWith("."), `模組必須使用相對路徑：${imported}`);
        const dependency = resolveWithin(
          output,
          path.relative(output, path.resolve(path.dirname(file), imported)),
        );
        assert.ok((await stat(dependency)).isFile(), `缺少匯入模組：${imported}`);
      }
    }
  }
}
...
assert.ok(
  (await artifactBytes(output)) <= 900 * 1024 * 1024,
  "網站產物超過 900 MiB 預留上限，請先處理容量再發布",
);
console.log(

export const ARTWORK_TYPES = { "image/png": "png", "image/jpeg": "jpg", "image/gif": "gif", "video/mp4": "mp4" };
...
export const ARTWORK_STATES = {
  draft: "草稿", queued: "等待處理", processing: "處理中", stored: "作品已保存",
  promoted: "等待部署", published: "已上線", failed: "處理失敗", cancelled: "已放棄", superseded: "已由新版本接續",
};
...
export function artworkOperation(value) {
  artworkAssert(typeof value === "string" && /^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/.test(value), "作品工作識別碼不正確。");
  return value;
}
export function artworkFields(input) {
  ...
  for (const [key, max, required] of [["title", 100, true], ["alt", 300, false], ["description", 2000, false]]) {
    const value = input[key] ?? "";
    artworkAssert(typeof value === "string" && value.length <= max && !/[\x00-\x08\x0b\x0c\x0e-\x1f]/.test(value), "作品文字格式不正確或過長。");
...
export function artworkFile(input) {
  ...
  artworkAssert(Number.isInteger(input.size) && input.size > 0 && input.size <= ARTWORK_MAX_BYTES, "作品每檔上限為 10 MiB。");
  artworkAssert(typeof input.name === "string" && input.name.length > 0 && input.name.length <= 150 && !/[\x00-\x1f\x7f\\/]/.test(input.name), "檔名不正確。");
...
export function artworkMime(bytes) {
  ...
  if (text.slice(4, 8) === "ftyp" && /isom|iso2|mp41|mp42|avc1|M4V |MSNV/.test(text.slice(8))) return "video/mp4";
  return "";
}
...
export function mergeArtworks(base, manifest, includeDeleted = false) {
  ...
  for (const item of manifest.items) {
    artworkAssert(/^(animation|chibi|stickers)-[0-9]+$/.test(item.id) && !seen.has(item.id), "作品編號重複或不正確。");
    seen.add(item.id);
    artworkAssert(Number.isInteger(item.revision) && item.revision > 0, "作品版本不正確。");
    const previous = map.get(item.id);
    map.set(item.id, { ...previous, ...item, ...item.media });
  }
  return [...map.values()].filter(work => includeDeleted || !work.deleted)

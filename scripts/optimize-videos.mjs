...
function run(tool, args) {
  return execFileSync(tool, args, { encoding: "utf8", windowsHide: true, stdio: ["ignore", "pipe", "pipe"] });
}
function probe(file) {
...
try {
  for (const work of works) {
    ...
    for (const [kind, videoProfile] of Object.entries(videoProfiles)) {
      if (saved?.sourceSha256 === work.sha256 && saved[kind] && JSON.stringify(previous.profiles?.[kind]) === JSON.stringify(videoProfile)) {
        try {
          const bytes = await readFile(resolveWithin(path.join(root, "public"), saved[kind].src));
          if (sha256(bytes) === saved[kind].sha256 && bytes.length === saved[kind].bytes && hasFastStart(bytes)) {
      ...
      run(ffmpeg, [
        ...
        "-vf", `scale=w='min(iw,${videoProfile.maxDimension})':h='min(ih,${videoProfile.maxDimension})':force_original_aspect_ratio=decrease:force_divisible_by=2:flags=lanczos`,
        "-c:v", videoProfile.codec, "-crf", String(videoProfile.crf),
        "-preset", videoProfile.preset, "-pix_fmt", videoProfile.pixelFormat,
        "-threads", "4", "-c:a", videoProfile.audio, "-movflags", "+faststart", target,
      ]);
      ...
      assert.equal(streams.filter((s) => s.codec_type === "audio").length, inputStreams.filter((s) => s.codec_type === "audio").length);
      ...
      asset[kind] = {
        src, bytes: bytes.length, sha256: hash,
        width: video.width, height: video.height, duration: Number(video.duration),
        frameRate: video.avg_frame_rate, frames: Number(video.nb_frames),
      };
  ...
  await writeFile(manifestPath + ".tmp", JSON.stringify({ version: 1, encoder, profiles: videoProfiles, videos }, null, 2) + "\n");
  await rename(manifestPath + ".tmp", manifestPath);
  for (const kind of Object.keys(videoProfiles))
    console.log(`已核對 ${videos.length} 支影片，${kind} 合計 ${videos.reduce((total, item) => total + item[kind].bytes, 0)} bytes；原始檔保留。`);
} finally {

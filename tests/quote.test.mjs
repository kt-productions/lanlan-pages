...
const quote = (
  items = [
    { label: "基本費", amount: 1200 },
    { label: "複雜費", amount: 150.25 },
  ],
...
function create(service = "chibi") {
  ...
  app.invoke("orders.submit", {
    ...
    details: submission({ service, chibiPlan: "animated", stickerIds: [1, 2] }),
  });
...
test("明細以整數分加總，支援折扣與零元，不採用客戶端總額", () => {
  const result = validateQuote(
    quote([
      { label: " 基本費 ", amount: "0.10" },
      { label: "加價", amount: 0.2 },
      { label: "折扣", amount: -0.01 },
    ]),
  );
...
test("拒絕空值、非金額、小數過多、負總額及過大明細，歷史單拒絕明細", () => {
  ...
  for (const items of [
    [],
    Array(51).fill({ label: "項目", amount: 1 }),
    [null],
    [{ label: " ", amount: 1 }],
    [{ label: "x".repeat(121), amount: 1 }],
    [{ label: "折扣", amount: -1 }],
    [
      { label: "費用", amount: 9999999.99 },
      { label: "加價", amount: 0.01 },
    ],
  ]) {
  ...
  assert.throws(() => validateQuote({ currency: "TWD", amount: -1, items: null }, true), {
    code: "VALIDATION",
  });
});
...
test("三種類型可保存明細並讀回，預估、收件回執及公開回應不混入管理報價", () => {
  for (const service of ["animation", "chibi", "stickers"]) {
    ...
    assert.deepEqual(
      app.invoke("admin.list", {}, token).data.orders[0].details.quote,
      result.data.details.quote,
    );
    const changed = app.invoke(
      "admin.update",
      { ...result.data, quote: quote([{ label: "調整後金額", amount: 1500 }]) },
      token,
    ).data;
  ...
  const details = validateSubmission(
    submission({ quote: quote(), estimatedPrice: { min: 1 }, priceConfirmed: true }),
    config,
  );
...
test("拖曳、舊管理頁及偽造 details.quote 不覆蓋已存報價；清除仍留下歷史", () => {
  ...
  const payload = {
    ...moved,
    details: { ...moved.details, quote: quote(), chibiPlan: "illustration" },
  };
...
test("報價沿用管理權限、版本衝突及寫入錯誤保護", () => {
  ...
  assert.equal(
    app.invoke("admin.update", { ...payload, quote: quote([{ label: "折扣", amount: -1 }]) }, token)
      .error.code,
    "VALIDATION",
  );
...
test("Trello 只新增手動總額，原始表單、來源、通知與預估空值完整保留", () => {
  const app = backend();
  app.context.importTrelloOrders_(
    prepareTrelloImport(
      [
        {
          service: "chibi",
          stages: { ["c".repeat(24)]: "queued" },
          board: {
            id: "b".repeat(24),
            name: "虛構看板",
            ...
            cards: [
              {
                ...
                idList: "c".repeat(24),
                name: "虛構歷史單",
                shortUrl: "https://trello.com/c/fixture",
                pos: 1,
                closed: false,
  ...
  const result = app.invoke(
    "admin.update",
    { ...order, details: submission(), quote: { currency: "TWD", amount: 888.88, items: null } },
    token,
  );
  assert.equal(result.ok, true);
  ...
  assert.equal(
    app.invoke("admin.update", { ...moved, quote: quote() }, token).error.code,
    "VALIDATION",
  );

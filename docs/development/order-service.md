...
草稿確認與等待付款合併顯示，舊值仍相容；公開與管理清單的未交稿依原始建立時間由舊到新，Trello 採來源卡片建立時間；已交稿依 `sortUpdatedAt` 由新到舊。此排序時間通常為本站 `updatedAt`，但 Trello 舊單的純金額／收益日期補齊不移動卡片，沒有其他本站更新時採來源最後活動時間，不用匯入及補資料時間排列歷史單。看板預設只載入未交稿，已交稿按需補載；收益報表另讀完整訂單快照。

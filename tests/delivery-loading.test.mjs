...
function fixture() {
  ...
  for (let index = 0; index < 100; index++) {
    ...
    row[columns.indexOf("status")] =
      index < 35 ? "drafting" : index % 2 ? "completed" : "delivered";
...
test("交稿範圍在伺服器分頁之前套用，保留封存界線及舊狀態對應", async () => {
  ...
  for (const admin of [false, true]) {
    ...
    assert.deepEqual(
      requests.map((payload) => payload.offset),
      admin ? [0, 30] : [0],
    );
    requests.length = 0;
  ...
  const page = app.invoke("progress.list", {
    delivery: "delivered",
    offset: 30,
    limit: 30,
    flag: "rush",
  ...
  assert.equal(
    app.invoke("progress.list", { delivery: "active", status: "delivered" }).data.total,
    0,
  );
...
test("公開未交稿超過一頁仍完整載入，且不要求已交稿", async () => {
  ...
  const orders = await loadBoardOrders(async (action, payload) => {
    ...
    return {
      orders: source.slice(payload.offset, payload.offset + 200),
      total: 205,
...
test("補載會取代已移動階段的同編號資料，並拒絕伺服器錯誤範圍", async () => {
  const previous = [
    { orderId: "LL-1", status: "queued" },
    { orderId: "LL-2", status: "drafting" },
  ];
  ...
  for (const [delivery, status] of [
    ["active", "delivered"],
    ["delivered", "queued"],
  ]) {
    await assert.rejects(
      loadBoardOrders(
        async () => ({ total: 1, nextOffset: null, orders: [{ orderId: "LL-1", status }] }),
        { delivery },
      ),
      /載入範圍/,
    );
  ...
  assert.deepEqual(
    await loadBoardOrders(async () => ({ total: 0, nextOffset: null, orders: [] }), {
      delivery: "delivered",
    }),
    [],
  );

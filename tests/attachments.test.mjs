...
function attachment(bytes = png, type = "image/png", name = "虛構測試.png") {
  return { name, type, size: bytes.length, sha256: createHash("sha256").update(bytes).digest("hex") };
}
function payload(count = 1) {
  return { requestId: randomUUID(), details: submission({ referenceUrl: "" }),
    attachments: Array.from({ length: count }, (_, index) => attachment(png, "image/png", `測試${index}.png`)) };
}
function upload(service, input) {
  input.attachments.forEach((_, index) => {
    const result = service.invoke("orders.upload", { ...input, index, data: png.toString("base64") });
...
test("五個檔案可取代素材連結，私人檔案完整保存且匿名端無法讀取", () => {
  ...
  assert.equal(service.invoke("admin.attachment", read, service.session()).data.base64, png.toString("base64"));
  assert.equal(service.invoke("admin.attachment", { ...read, index: 5 }, service.session()).ok, false);
  assert.doesNotMatch(JSON.stringify(service.invoke("progress.list")), /attachments|sha256|測試4|fixture-folder/);
...
test("第六個檔案、超過 10 MB、偽裝圖片、錯誤雜湊與缺少素材都遭拒絕", () => {
  ...
  assert.throws(() => attachmentManifest([{ ...attachment(), size: ATTACHMENT_MAX_BYTES + 1 }]), /10 MB/);
  assert.equal(attachmentManifest(Array(5).fill({ ...attachment(), size: 9 * 1024 * 1024 })).length, 5);
  assert.throws(() => attachmentManifest(Array(5).fill({ ...attachment(), size: 9 * 1024 * 1024 + 1 })), /45 MB/);
  const input = payload();
  for (const change of [{ data: Buffer.alloc(png.length).toString("base64") },
    { attachments: [{ ...attachment(), type: "image/jpeg" }] },
    { attachments: [{ ...attachment(), sha256: "0".repeat(64) }] }, { index: 9 }]) {
    assert.equal(service.invoke("orders.upload", { ...input, index: 0, data: png.toString("base64"), ...change }).ok, false);
  }
...
test("Drive 回應中斷與 Sheet 寫入失敗後重試沿用檔案及訂單識別碼", () => {
  ...
  assert.equal(service.invoke("orders.upload", { ...input, index: 0, data: png.toString("base64") }).ok, false);
...
test("上傳預留納入配額，暫停收件與未完成上傳不建立訂單", () => {
  ...
  assert.equal(service.invoke("orders.upload", { ...first, index: 0, data: png.toString("base64") }).ok, true);
  assert.equal(service.invoke("orders.submit", first).error.code, "ATTACHMENT");
  service.properties.set("DAILY_ORDER_LIMIT", "1");
  assert.equal(service.invoke("orders.upload", { ...payload(), index: 0, data: png.toString("base64") }).error.code, "RATE_LIMIT");
...
test("多人相簿部分失敗後只重試未送達者，送出期間不可重入", () => {
  ...
  service.faults.telegramResult = (_, url, options) => {
    count += 1;
    assert.ok(url.endsWith("/sendMediaGroup"));
    assert.ok(JSON.parse(options.payload.media).every(item=>!item.media.startsWith('attach://')));
    return response(200, { ok: true, result: Array(3).fill({photo:[{file_id:'fixture-file'}],media_group_id:'fixture-retry-group'}) });
  };
  assert.equal(service.invoke("admin.retryNotification", { orderId: order(service).orderId }, service.session()).data.status, "sent");
...
test("Telegram 明確拒絕圖片時改送原檔，管理員更新保留伺服器附件", () => {
  ...
  const input = payload();
  upload(service, input);
  service.faults.telegramResult = (_, url) => response(url.endsWith("/sendPhoto") ? 400 : 200,
    { ok: !url.endsWith("/sendPhoto") });
  service.invoke("orders.submit", input);
  const saved = order(service);
  assert.equal(saved.notificationStatus, "sent");
  assert.equal(service.calls.filter((call) => call.url.endsWith("/sendDocument")).length, 1);
  const result = service.invoke("admin.update", { orderId: saved.orderId, revision: 1, status: "queued",
    isRush: false, isOnHold: false, publicNote: "", adminNote: "", details: { ...input.details, attachments: [{ id: "forged" }] } }, service.session());
  assert.equal(result.ok, true, JSON.stringify(result));
...
test("一般檔案以不可執行內容保存並傳送文件；舊版訂單仍可編輯", () => {
  ...
  assert.equal(service.invoke("orders.upload", { ...input, index: 0, data: bytes.toString("base64") }).ok, true);
  service.invoke("orders.submit", input);
  assert.equal(JSON.parse(order(service).detailsJson).attachments[0].type, "application/octet-stream");
  assert.equal(service.calls.filter((call) => call.url.endsWith("/sendDocument")).length, 1);
  const saved = order(service);
  ...
  assert.equal(service.invoke("admin.update", { orderId: saved.orderId, revision: 1, status: "queued",
    isRush: false, isOnHold: false, publicNote: "", adminNote: "", details: previous }, service.session()).ok, true);
});
...
test("既有第三版連結表單可重試同一回執，但不能使用新版附件協定", () => {
  ...
  assert.equal(service.rows.length, 2);
  assert.equal(service.invoke("orders.upload", { ...input, attachments: [attachment()], index: 0, data: png.toString("base64") }).error.code, "VERSION");
});
...
test("相簿被明確拒絕時整組改送文件，成功結果保存同一群組，不拆成個別通知", () => {
  ...
  service.faults.telegramResult = (_, url, options) => {
    ...
    return count === 1 ? response(400, {ok:false}) : response(200, {ok:true,
      result: media.map((_,index)=>({document:{file_id:'fixture-doc-'+index},media_group_id:'fixture-album'}))});
  };
  service.invoke('orders.submit', input);
  assert.equal(count, 2);
...
test("舊版部分檔案已送達時，只把尚未送達圖片組成相簿", () => {
  ...
  states[0].parts[0] = {status:'sent',attempts:1,media:{method:'sendPhoto',fileId:'previous-photo'}};
  ...
  assert.equal(groups.length, 1);
  assert.deepEqual(JSON.parse(groups[0].options.payload.media).map(item=>item.media), ['attach://file_1','attach://file_2']);
...
test("相簿成功回應缺少部分訊息時標示結果不明，不自行拆開重送", () => {
  ...
  const input = payload(3);
  upload(service, input);
  service.faults.telegramResult = () => response(200, {ok:true,result:[{photo:[{file_id:'only-one'}]}]});
  service.invoke('orders.submit', input);

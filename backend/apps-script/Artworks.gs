...
function setupArtworkStorage() {
  ...
  return lock_(function () {
    const parent = setting_("ARTWORK_FOLDER_ID", true)
      ...
      const folder = Drive.Files.create(
        {
          ...
          parents: [parent],
          appProperties: { artworkStorage: "1" },
        },
        null,
        { fields: "id" },
      );
...
function artworkSheet_() {
  ...
  Core_.requireValue(
    sheet &&
      JSON.stringify(sheet.getRange(1, 1, 1, 2).getValues()[0]) ===
        JSON.stringify(ARTWORK_JOB_HEADERS_),
    "作品工作表尚未初始化或欄位不符。",
    "CONFIG",
  );
  return sheet;
...
function artworkJob_(operationId) {
  ...
  Core_.requireValue(
    job && job.siteId === artworkConfig_().site,
    "找不到這份作品工作。",
    "NOT_FOUND",
  );
  return job;
...
function artworkFolder_() {
  ...
  Core_.requireValue(
    !file.trashed &&
      ...
      file.appProperties?.artworkStorage === "1",
    "作品暫存資料夾設定不正確。",
    "CONFIG",
  );
  return id;
...
function artworkPublicJob_(job) {
  return {
    ...
    file: job.file ? ArtworkCore_.artworkFile(job.file) : null,
    uploaded: Boolean(job.file?.uploaded),
    error: job.error || "",
    cleanup: job.cleanup || "pending",
...
function listArtworks_() {
  ...
  return {
    works: ArtworkCore_.mergeArtworks(ARTWORK_BASE_, manifest),
    previewBase: "https://raw.githubusercontent.com/" + config.repo + "/" + sha + "/public/",
    jobs: artworkJobs_()
      .filter(function (job) {
        return job.siteId === config.site;
      })
      .map(artworkPublicJob_),
    maxBytes: ArtworkCore_.ARTWORK_MAX_BYTES,
  };
}
function saveArtwork_(payload) {
  ...
  return lock_(function () {
    ...
    let job = jobs.find(function (item) {
      return item.operationId === payload.operationId;
    });
    const inputHash = digest_(
      JSON.stringify({
    ...
    Core_.requireValue(
      !job || (job.state === "draft" && job.revision === payload.revision),
      "草稿已變更或已送出，請重新載入。",
      "CONFLICT",
    );
    Core_.requireValue(
      !job ||
        (job.work.category === fields.category &&
          job.sourceId === (payload.id || "") &&
          job.expectedRevision === payload.expectedRevision),
      "草稿的作品識別與基礎版本不能變更。",
      "CONFLICT",
    );
    Core_.requireValue(
      !job?.file || job.file.sha256 === file?.sha256,
      "已保存的草稿不能換檔，請另建草稿。",
      "CONFLICT",
    );
    ...
    Core_.requireValue(
      payload.id || (payload.action === "upsert" && file && payload.expectedRevision === 0),
      "新作品需要上傳檔案。",
    );
    Core_.requireValue(payload.action !== "delete" || !file, "下架不需上傳檔案。");
    const active = jobs.filter(function (item) {
      return !["published", "cancelled", "superseded"].includes(item.state);
    });
    Core_.requireValue(
      job || active.length < 30,
      "待處理作品已達 30 件，請先完成或清理草稿。",
      "CAPACITY",
    );
    const bytes = jobs
      .filter(function (item) {
        return item.cleanup !== "cleaned" && item.operationId !== payload.operationId;
      })
      .reduce(function (sum, item) {
        return sum + (item.file?.size || 0);
      }, 0);
    Core_.requireValue(
      bytes + (file?.size || 0) <= 200 * 1024 * 1024,
      "作品暫存已達 200 MiB，請先處理既有工作。",
      "CAPACITY",
    );
    ...
    if (!id) {
      ...
      const max = Math.max(
        ...
        ...catalog
          .filter(function (item) {
            return item.category === fields.category;
          })
          .map(function (item) {
    ...
    job = Object.assign(
      ...
      },
      {
        ...
        sourceId: payload.id || "",
        action: payload.action,
        work: Object.assign({ id: id }, fields),
        cleanup: file ? "pending" : "not_needed",
        expectedRevision: payload.expectedRevision,
        file:
          job?.file ||
...
function uploadArtwork_(payload) {
  ...
  return lock_(function () {
    ...
    Core_.requireValue(
      typeof payload.data === "string" && payload.data.length === Math.ceil(job.file.size / 3) * 4,
      "檔案大小不正確。",
    );
    const bytes = Utilities.base64Decode(payload.data);
    Core_.requireValue(
      bytes.length === job.file.size &&
        Utilities.base64Encode(bytes) === payload.data &&
        bytesDigest_(bytes) === job.file.sha256 &&
        ArtworkCore_.artworkMime(bytes) === job.file.type,
      "檔案格式或內容與草稿不符。",
    );
    ...
    const file = response
      ...
      : Drive.Files.create(
          ...
          Utilities.newBlob(bytes, job.file.type, job.file.name),
          { fields: fields },
        );
    Core_.requireValue(
      !file.trashed &&
        file.parents?.includes(folder) &&
        file.appProperties?.artworkOperation === job.operationId &&
        file.appProperties?.artworkSite === job.siteId &&
        Number(file.size) === job.file.size &&
        file.mimeType === job.file.type &&
        file.md5Checksum === bytesDigest_(bytes, Utilities.DigestAlgorithm.MD5),
      "無法確認作品保存結果，請重試同一檔案。",
      "ATTACHMENT",
    );
...
function publishArtwork_(payload) {
  ...
  const job = lock_(function () {
    ...
    Core_.requireValue(
      payload.publicConfirmed === true && (!current.file || payload.backupConfirmed === true),
      "請確認公開範圍及原檔備份。",
    );
  ...
  if (["queued", "failed", "stored", "promoted"].includes(job.state)) {
    try {
      ...
      return Object.assign(artworkPublicJob_(job), { dispatchPending: true });
    }
  }
  return artworkPublicJob_(job);
}
...
function artworkStoredFile_(job) {
  ...
  const file = JSON.parse(
    driveReferenceResponse_(
      job.file.id + "?fields=id,size,mimeType,parents,trashed,appProperties",
    ).getContentText(),
  );
  Core_.requireValue(
    !file.trashed &&
      file.parents?.includes(artworkFolder_()) &&
      file.appProperties?.artworkOperation === job.operationId &&
      file.appProperties?.artworkSite === job.siteId &&
      Number(file.size) === job.file.size &&
      file.mimeType === job.file.type,
...
function artworkChunk_(job, offset) {
  Core_.requireValue(
    Number.isInteger(offset) && offset >= 0 && offset < job.file?.size && offset % 262144 === 0,
    "作品區塊位置不正確。",
  );
  artworkStoredFile_(job);
  const end = Math.min(job.file.size - 1, offset + 262143);
  const response = UrlFetchApp.fetch(
    "https://www.googleapis.com/drive/v3/files/" + job.file.id + "?alt=media",
    {
      headers: {
  ...
  Core_.requireValue(
    [200, 206].includes(response.getResponseCode()),
    "作品區塊下載失敗。",
    "ATTACHMENT",
  );
...
function cancelArtwork_(payload) {
  ...
  return lock_(function () {
    ...
    const uncommittedFailure =
      job.state === "failed" &&
      !job.commitSha &&
      !catalog.some(function (work) {
        return work.operationId === job.operationId;
      });
    Core_.requireValue(
      (["draft", "cancelled"].includes(job.state) || uncommittedFailure) &&
        payload.confirmed === true,
      "已提交的工作請先重試發布；下架需從作品清單另行操作。",
      "CONFLICT",
    );
    job.state = "cancelled";
    writeArtworkJob_(job);
...
function cleanupArtwork_(payload) {
  return lock_(function () {
    const job = artworkJob_(payload.operationId);
    Core_.requireValue(
      job.commitSha && job.publicConfirmed && (!job.file || job.backupConfirmed),
      "原檔保存尚未確認。",
      "CONFLICT",
    );

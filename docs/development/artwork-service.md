...
`content/artworks.json` 可由維護提交設定正整數 `sortOrder`；數字越大越前，未指定者沿用 ID 數字。此欄位與配號、名稱分開，後台文字／媒體更新及下架保留既有值，目前編輯表單不提供排序輸入。舊版 GAS 的合併會保留此欄位，管理頁再套用共用排序，因此不需更動 API 請求或先更新 GAS。批次維護時仍提高 revision、更新操作識別，舊草稿須重新讀取後編輯；本次對照見[小動圖名稱與順序](../records/chibi-order-2026-09-24.md)。

| 位置 | 責任 |
| --- | --- |
| `src/pages/artworks/` | 獨立作品管理頁、已保存登入還原、到期與跨分頁登出；不讀取委託資料。 |

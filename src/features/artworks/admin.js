...
export function setupArtworkAdmin({ api, getToken, report }) {
  ...
  function preview(url, type, title) {
    ...
    const element = document.createElement(type === "video/mp4" || type === "video" ? "video" : "img");
    if (element.tagName === "VIDEO") { element.controls = true; element.playsInline = true; element.preload = "metadata"; }
  ...
  function setBusy(value) {
    ...
    $("artwork-fields").disabled = value || Boolean(draft && draft.state !== "draft") || action === "delete";
    $("artwork-confirmations").disabled = value;
  ...
  function render() {
    ...
    const visible = works.filter(work => (!category || category === work.category) && (!query || work.title.toLocaleLowerCase().includes(query)));
    $("artwork-list").replaceChildren(...visible.map(work => {
    ...
    const visibleJobs = jobs.filter(job => {
      ...
      return recent.has(job) || pendingCleanup || !["published", "superseded", "cancelled"].includes(job.state);
    });
    $("artwork-jobs").replaceChildren(...visibleJobs.reverse().map(job => {
      const button = document.createElement("button");
    ...
    $("artwork-status").textContent = `共 ${visible.length} 件作品；清單包含已保存的版本，發布紀錄顯示網站更新狀態。`;
  }
  async function load() {
    const epoch = generation;
    ...
    render();
    clearTimeout(timer);
    if (!panel.hidden && jobs.some(job => ["queued", "processing", "stored", "promoted"].includes(job.state))) {
      const poll = () => {
  ...
  function open(work, job, source) {
    ...
    for (const key of ["title", "category", "alt", "description"]) fields[key].value = item?.[key] || (key === "category" ? "chibi" : "");
    ...
    $("artwork-discard").hidden = !job || !["draft", "failed", "cancelled"].includes(job.state) || Boolean(job.commitSha);
    $("artwork-discard").textContent = job?.state === "cancelled" ? "重試清理草稿" : "放棄草稿";
    $("artwork-cleanup").hidden = !job?.commitSha || job.cleanup !== "pending";
    $("artwork-backup-label").hidden = !fileInfo && Boolean(work);
    $("artwork-delete-hint").hidden = action !== "delete";
    $("artwork-confirmations").hidden = Boolean(job && job.state !== "draft");
    $("artwork-discard-confirm").hidden = true;
    $("artwork-file-hint").textContent = job?.file ? `已保存：${job.file.name}。這份草稿的檔案已固定；需要換檔可放棄草稿後重新編輯。` : "PNG、JPG、GIF 或 MP4，每檔最多 10 MiB。GIF 最多 15 秒、MP4 最多 120 秒。編輯文字不必重傳。";
    setBusy(false);
    dialog.showModal();
  ...
  function close(force = false) {
    ...
    if (changed && !force) { message("尚有未儲存內容，請先儲存草稿，或按「放棄草稿」確認放棄。"); $("artwork-discard").hidden = false; return; }
    dialog.close();
    clearPreview();
    trigger?.focus();
  }
  async function save() {
    const work = artworkFields(Object.fromEntries(["title", "category", "alt", "description"].map(key => [key, fields[key].value])));
    if (!selected && !fileInfo) throw new Error("請選擇作品檔案。");
    const saved = await request("save", { operationId, revision: draft?.revision || 0, id: selected?.id || "",
      expectedRevision: draft?.expectedRevision ?? selected?.revision ?? 0, action, work, file: action === "delete" ? null : fileInfo });
    draft = saved;
  ...
  fields.file.addEventListener("change", () => task(async () => {
    const epoch = generation;
    ...
    $("artwork-backup-label").hidden = false;
    message("預覽已準備完成，儲存草稿後才會上傳。");
  }));
  form.addEventListener("submit", event => { event.preventDefault(); task(async () => { await save(); await load(); message("草稿已保存，尚未公開。"); }); });
  $("artwork-publish").addEventListener("click", () => task(async () => {
    if (!draft || draft.state === "draft") {
      if (!fields.publicConfirmed.checked || (fileInfo && !fields.backupConfirmed.checked)) throw new Error("請先確認公開範圍及原檔備份。");
      await save();
    }
    const result = await request("publish", { operationId, revision: draft.revision,
      publicConfirmed: fields.publicConfirmed.checked, backupConfirmed: fields.backupConfirmed.checked });
    changed = false;
    close(true);
    await load();
    message(result.dispatchPending ? "發布工作已保存，但 GitHub 尚未接收，請稍後從發布紀錄重試。" : "已送出發布，請從發布紀錄查看處理結果。");
  }));
  $("artwork-delete").addEventListener("click", () => {
    action = "delete"; fileInfo = null; file = null;
    fields.publicConfirmed.checked = false;
    $("artwork-backup-label").hidden = true;
  ...
  $("artwork-preview-load").addEventListener("click", () => task(async () => {
    const epoch = generation;
    const chunks = [];
    ...
    if (epoch !== generation) return;
    if (blob.size !== draft.file.size || hash !== draft.file.sha256) throw new Error("預覽完整性檢查失敗。");
    clearPreview(); objectUrl = URL.createObjectURL(blob); preview(objectUrl, draft.file.type, draft.work.title);
  }));
  $("artwork-discard").addEventListener("click", () => { $("artwork-discard-confirm").hidden = false; });
  $("artwork-cleanup").addEventListener("click", () => task(async () => {
    await request("cleanup", { operationId });
    changed = false; close(true); await load(); message("已核對原檔並清理暫存。");
  }));
  $("artwork-discard-no").addEventListener("click", () => { $("artwork-discard-confirm").hidden = true; });
  $("artwork-discard-yes").addEventListener("click", () => task(async () => {
    if (draft) await request("cancel", { operationId, confirmed: true });
    changed = false; close(true); await load();
  }));

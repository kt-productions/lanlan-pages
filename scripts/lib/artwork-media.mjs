...
export function run(command, args, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.stderr?.on("data", (bytes) => {
      if (errors.length < 12000) errors += bytes;
    });
...
export async function processArtwork(bytes, file, id, directory) {
  ...
  const frames = Number(
    counted.streams.find((item) => item.codec_type === "video").nb_read_frames ||
      stream.nb_frames ||
      1,
  );
  ...
  artworkAssert(
    Number.isFinite(frames) && frames > 0 && frames <= (animated ? 450 : video ? 7200 : 1),
    "作品影格數超出限制。",
  );
  artworkAssert(
    !(animated || video) || (duration > 0 && duration <= (animated ? 15 : 120)),
    "GIF 上限 15 秒，MP4 上限 120 秒。",
  );
  ...
  if (video) {
    for (const [name, size] of [
      ...
      await ffmpeg([
        ...
        "-an",
        "-vf",
        `${scale(size)},scale=trunc(iw/2)*2:trunc(ih/2)*2`,
        "-c:v",
        "libx264",
        "-preset",
        "medium",
  ...
  return {
    ...
    duration,
    src: source.path,
    poster: `${relative}/poster.png`,
    playbackSrc: video
      ? `${relative}/display.mp4`
      : animated
        ? source.path
        : `${relative}/display.png`,
    ...(video ? { previewSrc: `${relative}/preview.mp4` } : {}),
    source,
    assets,
  };
}

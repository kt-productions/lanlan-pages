...
const response = (code = 200) => ({
  ...
  getContentText: () => JSON.stringify({ ok: code === 200 }),
});
const texts = (app) =>
  app.calls
...
const saved = (app) =>
  Object.fromEntries(app.rows[0].map((key, index) => [key, app.rows[1][index]]));
...
test("動畫確認幣別後仍可重取舊回執及接續上傳，歷史金額不被改寫", () => {
  ...
  assert.equal(
    app.invoke("orders.upload", { ...pending, index: 0, data: bytes.toString("base64") }).ok,
    true,
  );
...
test("三類委託完整投影適用欄位，否與未詢問不同；預估沿用伺服器計價", () => {
  for (const service of ["animation", "chibi", "stickers"]) {
    ...
    for (const part of [
      ...
      "聯絡平台：Telegram",
      "聯絡方式：@fictional_test",
      "參考連結：https://example.com/reference",
      "可否當作品範例：是",
      "付款方式：PayPal",
      "已閱讀委託說明與製作流程：是",
      "計價明細：",
      "管理後台：https://example.com/lanlan-pages/admin/",
      formatPriceRange(
        clean.estimatedPrice.min,
        clean.estimatedPrice.max,
        clean.estimatedPrice.currency,
      ),
    ]) {
...
test("長網址、完整 48 款、4000 字需求與表情符號無截斷，逐段不超過 4096", () => {
  ...
  app.properties.set(
    ...
    Array.from({ length: 20 }, (_, i) => String(i + 1)).join(","),
  );
  submit(
    app,
    submission({
      service: "stickers",
      stickerIds: config.stickerOptions.map((item) => item.number),
      nickname: "暱".repeat(80),
      contact: { channel: "discord", value: "d".repeat(300) },
      referenceUrl: "https://example.com/" + "a".repeat(1980),
      notes: "😀".repeat(2000),
    }),
  );
  const chunks = texts(app)
    .filter((message) => message.chat_id === "1")
  ...
  assert.equal(
    chunks.map((text) => text.replace(/^委託內容 \d+\/\d+\n編號：[^\n]+\n\n/, "")).join(""),
...
test("文字失敗只補未送段落，管理修改與通知名單變動不改寫已保存的文字快照", () => {
  ...
  assert.equal(
    app.invoke(
      "admin.update",
      { ...order, details: { ...order.details, notes: "管理員修改後" } },
      token,
    ).ok,
    true,
  );
...
test("文字第一段回應不明時保留 unknown，不因後續未送段落而自動重傳", () => {
  ...
  assert.equal(
    app.invoke("admin.retryNotification", { orderId }, app.session()).data.status,
    "sent",
  );
});
...
test("文字與相簿獨立補送，五張圖片仍是一組且回執不重送已成功相簿", () => {
  ...
  for (let index = 0; index < 5; index++)
    assert.equal(
      app.invoke("orders.upload", { ...input, index, data: bytes.toString("base64") }).ok,
      true,
    );
  app.faults.telegramResult = (_, url, options) =>
    url.endsWith("/sendMessage")
      ...
      : {
          ...
          getContentText: () =>
            JSON.stringify({
              ok: true,
              result: JSON.parse(options.payload.media).map(() => ({
  ...
  app.faults.telegramResult = null;
  assert.equal(
    app.invoke("admin.retryNotification", { orderId: saved(app).orderId }, app.session()).data
      .status,
    "sent",
  );
...
test("使用者 HTML 與 Markdown 原樣傳送，敏感管理欄位及通知快照不進入公開 API", () => {
  ...
  const { orderId } = submit(
    app,
    submission({ nickname: "<b>虛構</b>", notes: "[範例](https://example.com) & <script>" }),
  );
  const message = texts(app)[0];
  ...
  assert.doesNotMatch(
    app.context.orderNotificationText_(order),
    /不可傳送|私密錯誤|fixture-bot|fixture-session/,
  );
...
test("舊版部分收件人已成功時不補寄，僅未成功者升級完整文字", () => {
  ...
  app.rows[1][col] = JSON.stringify([
    { id: "111", status: "sent", attempts: 1 },
    { id: "222", status: "failed", attempts: 1 },
  ]);
  app.faults.telegram = false;
  const before = texts(app).length;
  assert.equal(

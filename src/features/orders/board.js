...
export function filterBoard(snapshot, { service = "", status = "", flag = "", search = "" }) {
  ...
  const orders = snapshot.map((order) => ({ ...order, ...orderWorkflow(order) })).filter((order) =>
    ...
    (!needle || `${order.orderId} ${order.details?.nickname || order.displayTitle || ""}`.toLowerCase().includes(needle)))

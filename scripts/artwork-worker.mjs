...
artworkAssert(
  ...
  "此腳本只在受信任的 GitHub Actions checkout 執行。",
);
...
const git = (args, token = process.env.GITHUB_TOKEN) =>
  run("git", args, {
    cwd: root,
    env: {
      ...process.env,
...
async function processJob(job, leaseId) {
  ...
  try {
    ...
    if (!sha) {
      ...
      const base = [
        ...JSON.parse(await readFile(path.join(root, "content/works.json"), "utf8")),
        stickerWork,
      ];
      // 先驗證作品版本，再花時間下載與轉檔。
      ...
      await writeFile(
        path.join(root, "content/artworks.json"),
        JSON.stringify(next, null, 2) + "\n",
      );
      // 只清理受管目錄中已無引用的版本，舊素材、首頁與委託附件完全不在範圍內。
      const referenced = new Set(
        next.items.flatMap((item) => item.media?.assets.map((asset) => asset.path) || []),
      );
      ...
      for (const entry of await readdir(managed, { recursive: true, withFileTypes: true })) {
        ...
        const relative = path
          .relative(path.join(root, "public"), absolute)
          .split(path.sep)
          .join("/");
        managedPath(relative);
        if (!referenced.has(relative)) await rm(absolute);
      }
      for (const command of ["test", "build", "check"])
        await run("npm", ["run", command], { cwd: root, timeout: 300000 });
      phase = "提交 main";
      ...
      artworkAssert(
        changed.every(
          (file) => file === "content/artworks.json" || file.startsWith("public/assets/artworks/"),
        ),
        "作品提交超出允許範圍。",
      );
      await git([
        ...
        "user.name=作品發布服務",
        "-c",
        "user.email=artworks@users.noreply.github.com",
        "commit",
        "-m",
        "feat(artworks): 更新核可的公開作品",
        "-m",
    ...
    try {
      ...
      console.log("作品已保存；Drive 清理待重試，繼續發布。");
    }
    if (leaseError) throw leaseError;
    await call("heartbeat");
    phase = "同步 production";
    ...
    try {
      await promoteArtwork(git, sha, token, async () => {
        const response = await fetch(
          `https://api.github.com/repos/${process.env.GITHUB_REPOSITORY}/actions/workflows/pages.yml/dispatches`,
          {
            method: "POST",
            headers: {

...
function importTrelloOrders_(payload) {
  ...
  const candidates = payload.cards.map(function (card) {
    ...
    Core_.requireValue(
      source &&
        source.kind === "trello" &&
        /^[a-f0-9]{24}$/.test(source.cardId) &&
        /^[a-f0-9]{24}$/.test(source.boardId) &&
        /^[a-f0-9]{24}$/.test(source.listId) &&
        typeof source.cardName === "string" &&
        source.cardName.trim().length > 0 &&
        source.cardName.length <= 1000 &&
        typeof source.boardName === "string" &&
        source.boardName.length <= 1000 &&
        typeof source.listName === "string" &&
        source.listName.length <= 1000 &&
        ...
        typeof source.publishTitle === "boolean" &&
        Array.isArray(source.labels) &&
        source.labels.length <= 30 &&
        source.labels.every(function (label) {
          return typeof label === "string" && label.length <= 200;
        }) &&
        Array.isArray(source.attachments) &&
        source.attachments.length <= 100 &&
        source.attachments.every(function (attachment) {
          return (
            attachment &&
            typeof attachment.name === "string" &&
            attachment.name.length <= 1000 &&
            typeof attachment.url === "string" &&
            attachment.url.length <= 2000 &&
            /^https:\/\/trello\.com\/[^\s]+$/.test(attachment.url)
          );
        }) &&
        typeof source.lastActivity === "string" &&
        Number.isFinite(Date.parse(source.lastActivity)),
      "Trello 來源格式不正確。",
    );
    ...
    Core_.requireValue(
      ["animation", "chibi", "stickers"].includes(card.service) &&
        Object.hasOwn(Core_.ORDER_STATUSES, status) &&
        typeof card.isRush === "boolean" &&
        typeof card.isOnHold === "boolean",
      "匯入工作階段或類型不正確。",
    );
    ...
    const details = {
      recordType: "trello-import",
      service: card.service,
      nickname: source.cardName,
      contact: null,
      referenceUrl: null,
      notes: null,
      estimatedPrice: null,
    ...
    return Object.assign(detailColumns_(details), {
      ...
      publicVisible: true,
      isRush: card.isRush,
      isOnHold: card.isOnHold,
      publicNote: "",
      adminNote: "",
      lastEditor: "trello-import",
      ...
      historyJson: JSON.stringify([
        { at: now, actor: "trello-import", action: "imported", revision: 1 },
      ]),
    });
  });
  return lock_(function () {
    ...
    return {
      ...
      total: existing.length + pending.length,
      notifications: 0,
    };
  });
}
